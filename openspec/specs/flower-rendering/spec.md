...
### Requirement: Bush — multiple flowers at fixed canvas positions
A bush SHALL contain 1–3 flowers positioned at predefined SVG coordinate points:
- `PLANT_POINTS[0]` = [162, 231] — name flower
- `PLANT_POINTS[1]` = [80, 137] — date flower
- `PLANT_POINTS[2]` = [165, 60] — combination flower

A `marginTop` signal offsets all Y coordinates uniformly (default 20px).

#### Scenario: Three-flower bush
- **WHEN** both name and date inputs are provided
- **THEN** the bush SHALL render three flowers: name at [162, 231+marginTop], date at [80, 137+marginTop], combination at [165, 60+marginTop]

#### Scenario: Name-only bush
- **WHEN** only name is provided (no date)
- **THEN** the bush SHALL render one flower centered at `PLANT_CENTER` = [125, 150]
