...
- **THEN** the arc info panel SHALL display that arc's KOLIR, ČYSLO, and POJEDNANNJA info in the active language
- **AND** the KOLIR section background SHALL be the arc's `colorHex`
- **AND** the KOLIR section text SHALL be white or dark depending on the color's luminance to ensure readable contrast
...
- **WHEN** the KOLIR section is rendered
...
### Requirement: Arc info panel contains all 12 color descriptions in both languages
The panel lookup table SHALL include descriptions for all 12 colors (ordinals 1–12) in both Ukrainian Cyrillic and Latynka, sourced from `toneflower-translations.ts`.

#### Scenario: Every IDN color ordinal has a description in both languages
- **WHEN** the user hovers an arc whose color ordinal is any value from 1 to 12
- **THEN** the KOLIR section SHALL display a non-empty name and description in the active language

### Requirement: Arc info panel contains all 9 number meanings in both languages
The panel lookup table SHALL include meanings for all 9 numerological leaf numbers (1–9) in both languages.

#### Scenario: Every leaf number has a meaning in both languages
- **WHEN** the user hovers an arc whose leaf number is any value from 1 to 9
- **THEN** the ČYSLO section SHALL display a non-empty meaning text in the active language

### Requirement: Arc info panel contains all 36 color+number combination texts in both languages
The panel lookup table SHALL include a combined-energy text for each valid color+number pair in both languages.

#### Scenario: Known combination has POJEDNANNJA text in the active language
- **WHEN** the user hovers an arc with a color+number pair that has a defined combination entry
- **THEN** the POJEDNANNJA section SHALL display the combination text in the active language

#### Scenario: Panel is visually bright and readable
- **WHEN** the arc info panel is visible
- **THEN** it SHALL use a visually distinct design with clearly readable text
- **AND** KOLIR, ČYSLO, and POJEDNANNJA headers SHALL be visually distinct from body text
