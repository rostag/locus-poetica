...
Preset table (main bush, viewport ~250×400):
- Preset 0 — **Kompozytsija** (Reference artwork composition): `[[163,68],[73,148],[118,235]]`
...
#### Scenario: Main bush Preset 0 reflects the reference artwork composition
- **WHEN** the app loads for the first time (no localStorage entry)
- **THEN** the main bush SHALL render with Preset 0 coordinates matching the hand-painted reference image (upper-right, middle-left, lower-center triangle)
...
Preset table (Rozpakovka bush, viewport ~250×400):
- Preset 0 — **Kompozytsija** (Reference artwork composition): `[[132,60],[73,148],[178,148],[128,238]]`
...
#### Scenario: Rozpakovka Preset 0 reflects the reference artwork composition
- **WHEN** the app loads for the first time (no localStorage entry for Rozpakovka)
- **THEN** the Rozpakovka bush SHALL render with Preset 0 coordinates matching the hand-painted reference image (upper-center, middle-left, middle-right, lower-center diamond)
...
- **THEN** 5 SVG thumbnail preset buttons AND the "MOJA" button SHALL appear beneath BOTH the main bush SVG and the Rozpakovka bush SVG independently
